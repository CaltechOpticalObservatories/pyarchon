...
# @fn     set_param
# --------------------------------------------------------------------------
def set_param(param, value, cameras=None):
    """
    ...
    Args:
        ...
        cameras:
    """
    archon_open(cameras, do_load=False, do_power_on=False, do_setup=False)
...
def __send_command(*arg_list):
    ...
    if "expose" in arg_list:
        ...
        # to_time = int (np.ceil ((10000 + exptime + 0.1 * exptime) / 1000 ))
        to_time = int(np.ceil((100000 + exptime + 0.1 * exptime)))
