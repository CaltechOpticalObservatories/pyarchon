...
def expose(exptime=0, iterations=1):
    ...
    "iterations" loops over that many frames, requires support in the ACF.
    """
    # mode = caminfo.get_mode()

    caminfo.set_iterations(iterations)

    # if requested exptime is different then send a command to change it
    #
    if exptime != caminfo.get_exptime():
        print("setting new exposure time to", exptime)
...
def __send_command(*arg_list):
    ...
    for ii in range(0, numcams):
        ...
        while True:
            try:
                ready = select.select([sendsocket[ii]], [], [], to_time)
            except select.error:
                print("select error")
                break
            if ready[0]:
        ...
        dat[ii] = message
        print("[DEBUG] command=", command, " return=", dat[ii])
...
def setled(volts_in):
    """
    equivalent to bias(9, 25, volts_in)
    """

    error = bias(9, 25, volts_in)[0]
